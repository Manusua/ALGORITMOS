                                ...
                                int incr, int n_perms)
{
  ...
  if(metodo == NULL || fichero == NULL || num_min < 0 || num_max > 0 || incr < 0 || n_perms < 0) return ERR;  
  ...
  int incr, int n_veces)
{
  ...
  if(fichero== NULL || num_min < 0 || num_max < 0 || n_veces < 0 || incr < 0 || metodo == NULL || generador == NULL) return ERR;
  if(orden != ORDENADO && orden != NO_ORDENADO) return ERR;
  ...
  for (i = 0; i < numV; ++i) {
    ...
    if (
      ...
      printf("%d de %d Completada %d\n", i, numV, num_min + i*incr);
  }
  ...
  PTIEMPO ptiempo)
{
  ...
  if(n_veces < 0 || N < 0 || ptiempo == NULL || metodo == NULL || generador == NULL) return ERR;
  ...
  ptiempo->tiempo = ((1.0)*(t2-t1)/(N*n_veces)/CLOCKS_PER_SEC);
  libera_diccionario(dicc);
  free(perm);
  free(tabla);
  free(ppos);
